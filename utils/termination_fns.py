...
def termination_fn_halfcheetah(obs, act, next_obs):
    ...
    not_done = torch.logical_and(torch.all(next_obs > -100, axis=-1), torch.all(next_obs < 100, axis=-1))
    done = ~not_done
    done = done[:, None]
    return done
...
def termination_fn_halfcheetahveljump(obs, act, next_obs):
    ...
    done = torch.array([False]).repeat(len(obs))
    done = done[:,None]
    return done
...
def terminaltion_fn_door(obs, act, next_obs):
    ...
    done = torch.array([False] * obs.shape[0])
